...
- Generated manifest and marketplace files come from `tooling/generate.ts`; edit
  the config there, then run `bun run generate`.
- The canonical skill is `skills/share-video/SKILL.md`; `.agents/skills` only
  points Devin-compatible discovery at that same content.
- Keep host-specific generated metadata in the host manifest folders. Do not add
  package-local docs or extra top-level markdown unless the user asks; improve
