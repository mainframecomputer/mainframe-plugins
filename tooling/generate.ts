...
// `.mcp.json` is the single source of truth for the hosted MCP wiring. Cursor,
...
// Hermes has no plugin manifest that bundles MCP and skills the way the other
// hosts do: MCP servers are declared in the user's `~/.hermes/config.yaml`, and
// skills load from a directory. So the Hermes "plugin" is a generated MCP config
// fragment users merge in, plus the shared `share-video` skill (loaded via
// `hermes skills install` or `skills.external_dirs` — see README.md). Hermes
// ships no stop hook: no Hermes stop event's return value re-engages the agent
// (stop-time hooks are observers; shell hooks, the hooks/core analog, only act
// on pre_tool_call/pre_llm_call), so unlike Cursor/Codex/Claude there is nothing
// for the host-agnostic stop runtime to drive.
const HERMES_CONFIG = ".hermes-plugin/config.yaml";
const HERMES_CONFIG_HEADER = [
  "# Mainframe Hermes plugin wiring — generated by tooling/generate.ts; do not edit by hand.",
  "# Merge into ~/.hermes/config.yaml. See README.md for the share-video skill setup.",
  "",
].join("\n");
...
function main(): void {
  ...
  writeYaml(HERMES_CONFIG, HERMES_CONFIG_HEADER, hermesConfig());
...
function updatePackageJson(): void {
  ...
  packageJson.description =
    "Mainframe Cursor, Codex, Claude Code, and Hermes plugin manifests, skill, MCP wiring, and stop hooks.";
...
function writeYaml(path: string, header: string, value: unknown): void {
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, `${header}${stringifyYaml(value)}`);
}
