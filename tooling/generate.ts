...
// Hermes has no plugin manifest that bundles MCP, skills, and hooks the way the
// other hosts do: MCP servers and shell hooks are declared in the user's
// `~/.hermes/config.yaml`. So the Hermes "plugin" is a generated config
// fragment users merge in. The stop nudge runs as a `pre_llm_call` shell hook —
// Hermes invokes the command, pipes the turn payload to stdin, and prepends any
// returned `{"context": ...}` to the user message (see hooks/hermes/stop.ts).
const HERMES_CONFIG = ".hermes-plugin/config.yaml";
const HERMES_HOOK_BIN = "mainframe-hook-hermes";
const HERMES_HOOK_EVENT = "pre_llm_call";
...
function hermesConfig() {
  ...
  return {
    ...
    hooks: {
      [HERMES_HOOK_EVENT]: [{ command: HERMES_HOOK_BIN, timeout: HERMES_HOOK_TIMEOUT_SECONDS }],
    },
  };
}
...
function updatePackageJson(): void {
  ...
  packageJson.bin = {
    ...
    "mainframe-hook-hermes": "./dist/hooks/hermes/stop.js",
  };
